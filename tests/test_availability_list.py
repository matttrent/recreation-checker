...
def test_permit_availability_list(permit_availability_list):
    ...
    assert pal.availability[0] == PermitAvailability(
        "290", dt.date(2022, 7, 1), total=14, remaining=0, is_walkup=True
    )
    assert pal.availability[1] == PermitAvailability(
        "290", dt.date(2022, 7, 2), total=14, remaining=1, is_walkup=False
    )
    assert pal.availability[4] == PermitAvailability(
        "291", dt.date(2022, 7, 1), total=14, remaining=0, is_walkup=True
    )
...
def test_permit_inyo_availability_list(permit_inyo_availability_list):
    ...
    assert pal.availability[0] == PermitAvailability(
        "424", dt.date(2022, 7, 1), total=8, remaining=5, is_walkup=False
    )
    assert pal.availability[4] == PermitAvailability(
        "425", dt.date(2022, 7, 2), total=10, remaining=2, is_walkup=False
    )
    assert pal.availability[8] == PermitAvailability(
        "441", dt.date(2022, 7, 3), total=900000, remaining=900000, is_walkup=False
    )
