...
def permit_division_data():
    return {
        ...
        "description": "This zone is in the center of the Rockbound Valley with the Rubicon River "
...
def test_permit_availability_init(permit_availability_data):
    ...
    dates = set(dt.datetime(2022, 7, i, tzinfo=dt.timezone.utc) for i in range(1, 5))

    for divis in divisions:
        assert dates == set(permit_avail.availability[divis].date_availability.keys())
...
def permit_inyo_availability_data() -> dict[str, Any]:
    return {
        "payload": {
            "2022-07-01": {
                "424": {"total": 8, "remaining": 5, "is_walkup": False},
                "425": {"total": 10, "remaining": 2, "is_walkup": False},
                "441": {"total": 900000, "remaining": 900000, "is_walkup": False},
            },
            "2022-07-02": {
                "424": {"total": 8, "remaining": 4, "is_walkup": False},
                "425": {"total": 10, "remaining": 2, "is_walkup": False},
                "441": {"total": 900000, "remaining": 900000, "is_walkup": False},
            },
            "2022-07-03": {
                "424": {"total": 8, "remaining": 5, "is_walkup": False},
                "425": {"total": 10, "remaining": 6, "is_walkup": False},
                "441": {"total": 900000, "remaining": 900000, "is_walkup": False},
            },
        }
    }
...
def test_permit_inyo_availability_init(permit_inyo_availability_data):
    ...
    dates = set(dt.date(2022, 7, i) for i in range(1, 4))
